require 'byebug'
class PathPlotter
  attr_accessor :points, :position_x, :position_y, :positions, :instructions
  ...
  def plot
    instructions.each do |instruction|
      ins = Instruction.new(instruction)
      ins.x_diff.abs.times do
        self.position_x += ins.x_diff > 0 ? 1 : -1
        self.positions << [position_x, position_y]
      end
      ins.y_diff.abs.times do
        self.position_y += ins.y_diff > 0 ? 1 : -1
        self.positions << [position_x, position_y]
      end
    end
  end
...
class Instruction
  ...
  def initialize(input)
    self.direction = input.chars.first
    self.distance = input[1..-1].to_i
  end

  def x_diff
    return distance * -1 if direction == 'L'
    return distance if direction == 'R'

    0
  end

  def y_diff
    return distance * -1 if direction == 'D'
    return distance if direction == 'U'
...
class Crossings
  attr_accessor :path_a, :path_b
  def initialize(a, b)
    self.path_a = PathPlotter.new(a).positions
    self.path_b = PathPlotter.new(b).positions
  end
  ...
  def distance
    find.collect { |pos| manhattan([0, 0], pos) }.min
  end
  ...
  def steps
    find.collect { |pos|
      path_a.index(pos) + path_b.index(pos) + 2
    }.min
  end
end
...
class PathPlotterTest < Minitest::Test
  def test_position
    p = PathPlotter.new('R8,U5,L5,D3')
    assert_equal 3, p.position_x
    assert_equal 2, p.position_y

    p = PathPlotter.new('U7,R6,D4,L4')
    assert_equal 2, p.position_x
    assert_equal 3, p.position_y
  end
end

class CrossingsTest < Minitest::Test
  def test_position
    c = Crossings.new('R8,U5,L5,D3', 'U7,R6,D4,L4')
    assert_equal 6, c.distance
  end

  def test_steps
    c = Crossings.new('R8,U5,L5,D3', 'U7,R6,D4,L4')
    assert_equal 30, c.steps
  end
end
